...
export class TradeResolverBase {
  ...
  async createTrade(@graphql.Args() args: CreateTradeArgs): Promise<Trade> {
    return await this.service.createTrade({
      ...
      data: {
        ...
        arbitragePath: args.data.arbitragePath
          ? {
              connect: args.data.arbitragePath,
            }
          : undefined,
      },
    });
  ...
  @graphql.ResolveField(() => ArbitragePath, {
    nullable: true,
    name: "arbitragePath",
  })
  async getArbitragePath(
    @graphql.Parent() parent: Trade
  ): Promise<ArbitragePath | null> {
    const result = await this.service.getArbitragePath(parent.id);
